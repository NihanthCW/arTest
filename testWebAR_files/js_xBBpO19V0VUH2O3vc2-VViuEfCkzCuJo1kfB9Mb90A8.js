...
$(document).ready(function(){
  ...
  //pages cached in drupal so alert is still showing after dismissal
  let $alert = $('header .callout');

  if(!$alert || !$alert.is(':visible')){
    return;
  }

  let cookie_date = Cookies.get('Drupal.visitor.ucar-alert');

  //no cookie? hasn't been dismissed
  if(cookie_date === undefined) return;

  //alert updated? delete cookie
  let changed_date = $alert.attr('data-changed');

  if(cookie_date != changed_date) {
    Cookies.remove('Drupal.visitor.ucar-alert');
  }
